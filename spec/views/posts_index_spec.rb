...
RSpec.describe 'posts#index', type: :feature do
  let(:first_user) { User.create!(name: 'Aladdin', photo: 'https://unsplash.com/photos/a-group-of-people-holding-a-teddy-bear-together-FAqkG14YOKM', bio: 'Aladdin\'s biography', post_counter: 0) }
  let!(:post1) do
  ...
  it 'displays the number of posts through the post_counter' do
    expect(page).to have_content("Comments: #{post4.ComentsCounter}")
  end
  ...
  it 'displays the latest five comments of a post truncated to 90 characters' do
    post1.five_latest_comments.each do |c|
  ...
  it 'should redirect to the post show page through the View post button' do
    click_link(href: user_posts_path(first_user, post1))
    expect(page).to have_current_path(user_posts_path(first_user, post1.id))
  end
